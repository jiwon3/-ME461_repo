...
__interrupt void RXAINT_recv_ready(void)
{
    ...
    // if 'a' is entered to the serial port, the global variable RXAdata is updated to become a. When RXAdata is a, turn on the red LED.
    // When RXAdata is equal to b (when you type b into the serial port), turn off the red LED.
    if (RXAdata == 'a'){
